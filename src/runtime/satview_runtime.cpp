...
SatViewRuntime::SatViewRuntime()
    : view_controller_(std::make_unique<SatViewViewController>())
...
void SatViewRuntime::draw(SatViewFrameSink& frame)
{
    ...
    const float earth_scene_radius = satview_visible_scene_radius({
        .tracks = snapshot && snapshot->tracks
            ? std::span<const SatelliteOrbitTrack>(*snapshot->tracks)
            : std::span<const SatelliteOrbitTrack>{},
        .states = snapshot
            ? std::span<const SatellitePropagatedState>(snapshot->states)
            : std::span<const SatellitePropagatedState>{},
        .next_teme_positions_km = snapshot
            ? std::span<const glm::dvec3>(snapshot->next_teme_positions_km)
            : std::span<const glm::dvec3>{},
        .filter = &filter_,
        .source_label = snapshot ? std::string_view(snapshot->source_label) : std::string_view{},
        .selected_id = selected_norad_catalog_id_,
        .track_display_mode = track_display_mode_,
        .satellite_display_mode = satellite_display_mode_,
    });
    ...
    const float scene_radius = generic_body_view
        ? satview_solar_system_scene_radius(camera_pov_)
        : camera_scene_radius(
              earth_scene_radius,
              moon,
              sun_position,
    ...
    {
        std::vector<SatViewSceneVertex> earth_track_vertices = earth_track_visible
            ? compose_satview_earth_track(*earth_track_center_seconds_, track_sample_count_)
            : std::vector<SatViewSceneVertex>{};
        ...
        || lunar_tracks_need_reanchoring)
    {
        ...
        if (show_tracks)
        {
            ...
            {
                track_vertices = compose_satview_natural_body_tracks(
                    camera_pov_,
                    planet_tracks_,
                    track_sample_count_);
                auto rings = compose_satview_planetary_rings(camera_pov_);
                track_vertices.insert(track_vertices.end(), rings.begin(), rings.end());
            }
            else if (snapshot && snapshot->tracks)
            {
                track_vertices = compose_satview_tracks({
                    .tracks = *snapshot->tracks,
                    .filter = &filter_,
                    .source_label = snapshot->source_label,
                    .selected_id = selected_norad_catalog_id_,
                    .display_mode = track_display_mode_,
                    .color_mode = color_mode_,
                    .projection_mode = projection_mode_,
                    .camera_pov = camera_pov_,
                    .simulation_seconds = simulation_seconds,
                    .ground_observer_render_position = ground_context,
                    .ground_horizon_occlusion = ground_horizon_occlusion_,
                });
            }
            if (show_moon_track)
            {
                auto moon_track = compose_satview_moon_track(
                    *moon_track_center_seconds_, track_sample_count_);
                track_vertices.insert(
...
{
    const SatViewPovTransition transition = view_controller_->change_pov(
        camera_pov_, pov, projection_mode_, filter_);
    if (transition.central_body == CentralBody::Moon)
        moon_enabled_ = true;
    satview_select_central_body(filter_, transition.central_body);
    if (pov == SatViewCameraPov::Sun)
        sun_enabled_ = true;
    projection_mode_ = transition.projection;
    if (!transition.pov_changed)
    {
        if (transition.central_body_changed)
        {
            simulation_settings_dirty_ = true;
            invalidate_visual_buffers();
            request_redraw();
...
{
    map_center_radians_ = view_controller_->pan_map(map_center_radians_, delta_radians);
    request_redraw();
}

void SatViewRuntime::enter_ground_view_at(glm::dvec2 longitude_latitude_radians)
{
    const SatViewGroundEntry transition = view_controller_->enter_ground(
        longitude_latitude_radians);
    projection_mode_ = transition.projection;
    camera_pov_ = transition.pov;
    satview_select_central_body(filter_, transition.central_body);
    ground_location_radians_ = transition.location_radians;
    ground_camera_orientation_ = satview_default_ground_camera_orientation();
    camera_->ClearMotion();
    camera_manipulator_->Cancel();
    camera_keys_->reset();
...
std::size_t SatViewRuntime::visible_track_count(const SatViewSimulationSnapshot* snapshot) const
{
    ...
    for (const SatelliteOrbitTrack& track : *snapshot->tracks)
    {
        ...
        if (!satview_filter_matches(
                filter_, make_satview_filter_candidate(track, source_label)))
            continue;
...
void SatViewRuntime::apply_selection(const SatViewSelectionState& selection)
{
    selected_norad_catalog_id_ = selection.satellite_id;
    selected_natural_body_ = selection.natural_body;
    selected_surface_object_ = selection.surface.has_value()
        ? std::optional<SelectedSurfaceObject>(SelectedSurfaceObject{
              selection.surface->body,
              selection.surface->catalog_index,
          })
        : std::nullopt;
}
...
void SatViewRuntime::select_nearest_object(const glm::ivec2& screen_pos)
{
    ...
    const float earth_scene_radius = satview_visible_scene_radius({
        ...
        .next_teme_positions_km = snapshot->next_teme_positions_km,
        .filter = &filter_,
        .source_label = snapshot->source_label,
        .selected_id = selected_norad_catalog_id_,
        .track_display_mode = track_display_mode_,
    ...
    {
        apply_selection(view_controller_->select_satellite(nearest_id));
        sync_simulation_render_settings();
        invalidate_visual_buffers();
    }
    request_redraw();
...
bool SatViewRuntime::select_nearest_natural_body(const glm::ivec2& screen_pos, bool enter_body)
{
    ...
    const int pixel_h = std::max(1, scene_viewport_.pixel_size.y);
    const float viewport_aspect = static_cast<float>(pixel_w) / static_cast<float>(pixel_h);
    const float scene_radius = satview_solar_system_scene_radius(camera_pov_);
    const glm::mat4 view = camera_view_matrix(*camera_);
    const glm::mat4 proj = reversed_z_perspective(
        glm::radians(camera_->GetFieldOfView()),
        viewport_aspect,
    ...
    apply_selection(view_controller_->select_natural_body(*nearest_body));
    invalidate_visual_buffers();
    if (enter_body)
...
bool SatViewRuntime::select_nearest_surface_object(const glm::ivec2& screen_pos)
{
    ...
    if (!nearest_index.has_value())
        return false;
    apply_selection(view_controller_->select_surface(body, *nearest_index));
    sync_simulation_render_settings();
    invalidate_visual_buffers();
    request_redraw();
    return true;
