...
- [ ] Land current observatory work and retain the delivered SatView host smoke coverage; prefer the boundaries in `kanban/pending/06 satview-scene-composer-host-split -refactor.md` if already underway.
- [ ] Add a versioned SatView-owned bookmark store separate from general defaults; use the shared core atomic-write policy.
...
Depends on active SatView work and delivered host-smoke coverage; easier after `kanban/pending/06 satview-scene-composer-host-split -refactor.md` separates host/config from renderer.
