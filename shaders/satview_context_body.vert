...
void main()
{
    ...
    const float u = (float(lon) + corner.x) / float(lon_bands);
    const float v = (float(lat) + corner.y) / float(lat_bands);
    const float theta = u * 2.0 * PI + push.render_params.z;
    ...
    const vec3 scaled = vec3(sphere.x, sphere.y * polar_ratio, sphere.z);
    const vec3 normal = normalize(vec3(sphere.x, sphere.y / polar_ratio, sphere.z));
    const vec3 world = push.camera_orientation.xyz
        + scaled * push.camera_orientation.w;
