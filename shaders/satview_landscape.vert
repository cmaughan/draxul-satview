...
layout(location = 2) in vec4 in_local_direction2;
...
vec4 project_landscape_direction(vec3 direction)
{
    vec3 world_position = push.camera_pos.xyz
        + direction * LANDSCAPE_DISTANCE_EARTH_RADII;
    return satview_project_world_position(world_position);
}
...
void main()
{
    ...
    bool primitive_visible = landscape_projection_valid(directions[0], projected[0])
        ...
        && landscape_projection_valid(directions[2], projected[2]);
